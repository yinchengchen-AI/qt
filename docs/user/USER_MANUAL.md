...
> 客户 / 合同 / 开票 / 回款 一体化管理
> 文档版本:对应系统 v0.12.0(2026-07-28)
> 最近同步:2026-07-28(前端移动端适配)
...
| 数据字典 | CRUD | R | R | R | R |
| 公告 | CRUD | R | R | CRUD (改/删仅自己发的) | R |
| 操作日志 | R | — | — | — | — |
| 消息中心 | CRUD | CRUD | CRUD | CRUD | CRUD |
| 回收站 (v0.18.0+ service 守门) | CRUD | (R 列表 + 行级隔离) | (R 列表) | (R 列表) | (R 列表 + 行级隔离) |

> **C**=Create 新建, **R**=Read 查看, **U**=Update 编辑, **D**=Delete 删除。

**v0.18.0 (2026-08-02) 调整** — 详见 [CHANGELOG.md v0.18.0](../../CHANGELOG.md) 与 [docs/history/security/permissions-audit-2026-08-02.md](../history/security/permissions-audit-2026-08-02.md):
- DUNNING 矩阵重排: 现场 (SALES/EXPERT) 只 CR; **财务拿 CRUD**.
- EXPERT 不再开票: 仅 R + 导出, 商业发起归 SALES.
- 公告按发文主体管理: 仅发布人/ADMIN 可改/删 (service 强制).
- 回收站 service 入口硬卡 ADMIN; 菜单可绕过性已消除.
...
- **公告按发文主体管理** (v0.18.0+): OPS / ADMIN 发出的公告, 仅发布人或管理员可改/删; 同一角色内其他账号不可编辑他人公告 (service 强制, 见 §3.2 注).
...
- **R-09**:`→ ISSUED` 时:电子发票号必须 **20 位数字**,不合规拒绝 `INVOICE_INFO_INVALID`;公司抬头的税号为选填
...
档案编辑分 5 步,每步只填该步的字段;每步可点「保存本步」单独保存,也可在最后一步「提交」统一保存:
