...
### [P2-7] 缺 e2e 测试隔离
- **文件**: `tests/e2e-flow.mjs` 当前每次跑都建新数据（不清理）
- **问题**: 累计跑了 27 次会留 ~27 个客户；Sequence 编号会涨到 0027
- **建议**: 增加 `tests/setup.ts` 在测试开始前 truncate 表（或用 SQLite 内存库）。
