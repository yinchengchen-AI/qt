...
- **change(messages)**: 删除左侧分类侧栏(6 个大按钮),分类筛选收敛为工具栏内紧凑 Select(未读计数以文本展示),布局由双栏改单列。
- **change(messages)**: 主列表删除独立「状态」列,未读改为行内红点 + 标题加粗表达,列噪减少一列。
- **change(messages)**: 移动端复用同一分类 Select,删除专用 `SelectCategoryMobile` 组件。
- **change(messages)**: PageHeader 按钮顺序调整为「全部已读 / 订阅设置 / 清除已读」,公告 Tab 仍隐藏消息专属操作。

> 纯前端 UI 重构:API、数据模型、公告组件、Tabs(含公告/归档/回收站)与 deep link 均不变。
