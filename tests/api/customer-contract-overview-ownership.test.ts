...
// 报 500。根因:Project / Invoice / Payment 模型无 `ownerUserId` 字段(分别用
// `managerUserId` / `applicantUserId` / `recorderUserId`),overview 函数却 spread
// 了 `ownerEq(user)`,Prisma 把 `ownerUserId` 当 unknown arg 抛 P2009,被
// `lib/api.ts:err()` 兜底成 500。
// 正确做法:这三处跨一跳查询改走 `ownerViaContract(user)`,经 contract 关系过
// 滤 ownerUserId。
...
describe("客户/合同概览的 ownership 过滤(避免 SALES 报 ownerUserId 未知字段)", () => {
  ...
  it("getCustomerOverview 内 Project 查询用 ownerViaContract,不再 spread ownerEq", () => {
    const src = read("server/services/customer.ts");
    const overviewBody = src.match(
      /export\s+async\s+function\s+getCustomerOverview[\s\S]*?\n\}\s*\n/
    )?.[0] ?? "";
    expect(overviewBody, "应能定位 getCustomerOverview 函数体").toBeTruthy();

    const projectBlock = overviewBody.match(
      /prisma\.project\.findMany\(\s*\{[\s\S]*?\}\s*\)/
    )?.[0] ?? "";
    expect(projectBlock, "应能找到 prisma.project.findMany 调用").toBeTruthy();
    expect(projectBlock, "Project 查询应 spread ownerViaContract").toMatch(
      /\.\.\.\(?\s*ownerViaContract\(user\)/
    );
    expect(projectBlock, "Project 查询不应再 spread ownerEq").not.toMatch(
      /\.\.\.\s*ownerEq\(user\)/
    );
  });
  ...
  it("getContractOverview 内 Project / Invoice / Payment 查询都用 ownerViaContract", () => {
    const src = read("server/services/contract.ts");
    const overviewBody = src.match(
    ...
    for (const model of ["project", "invoice", "payment"]) {
      const block = overviewBody.match(
        new RegExp(`prisma\\.${model}\\.findMany\\(\\s*\\{[\\s\\S]*?\\}\\s*\\)`)
      )?.[0] ?? "";
      expect(block, `应能找到 prisma.${model}.findMany 调用`).toBeTruthy();
  ...
  it("ownerViaContract 应被 cast 到对应 model 的 WhereInput(避免 Prisma 泛型过严报错)", () => {
    ...
    expect(customerSrc).toMatch(/ownerViaContract\(user\)\s*as\s*Prisma\.ProjectWhereInput/);
    ...
    expect(contractSrc).toMatch(/ownerViaContract\(user\)\s*as\s*Prisma\.ProjectWhereInput/);
