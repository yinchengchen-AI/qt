...
import { renderPrintHtml, type PrintDoc, type PrintTableSection } from "@/lib/print-html";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [manager, progressLogs, taskInstances] = await Promise.all([
      p.managerUserId
        ? prisma.user.findUnique({ where: { id: p.managerUserId }, select: { name: true, employeeNo: true } })
      ...
      prisma.workflowTaskInstance.findMany({
        ...
        select: { attachments: true, task: { select: { name: true, code: true } } }
      })
    ]);
    ...
    const ATTACH_BY_ID = new Map<string, { id: string; name: string; mimeType: string; size: number; tasks: string[]; uploadedAt?: string }>();
    for (const ins of taskInstances) {
      const taskLabel = ins.task ? `${ins.task.name}${ins.task.code ? "(" + ins.task.code + ")" : ""}` : "(未关联任务)";
    ...
    const projectAttachments = Array.from(ATTACH_BY_ID.values()).sort((a, b) => (b.uploadedAt ?? "").localeCompare(a.uploadedAt ?? ""));
    ...
    const phaseSection: PrintTableSection = {
      ...
      rows: stats.byPhase.map((ph) => {
        const pct = ph.total > 0 ? ((ph.completed / ph.total) * 100).toFixed(1) + "%" : "—";
        return {
          阶段: ph.name,
          任务数: String(ph.total),
          已完成: String(ph.completed),
          完成率: pct,
          状态: ph.locked ? "锁定(前置未完)" : ph.total > 0 && ph.completed === ph.total ? "已完成" : "进行中"
        };
      })
    };
    ...
    const historySection: PrintTableSection = {
      ...
      rows: history.items.slice(0, 30).map((h) => ({
        ...
        任务: h.taskName ? `${h.taskName}${h.taskCode ? "(" + h.taskCode + ")" : ""}` : "项目级",
    ...
    const doc: PrintDoc = {
      ...
      mainRows: [
        ...
        { label: "项目进度", value: `${progressPct.toFixed(1)}%(基于工作流任务完成度)` },
        { label: "创建时间", value: fmtDateTime(p.createdAt) }
      ],
      summary: [
        { label: "任务总数", value: String(stats.totalTasks), tone: "primary" },
