...
import { getProject, updateProject, softDeleteProject } from "@/server/services/project";
...
export async function DELETE(
  ...
  return runWithRequestContext(_req, async () => {
    try {
      const user = await requireSession();
      const { id } = await params;
      const data = await softDeleteProject(user, id);
      return ok(data);
    } catch (e) {
      return err(e);
    }
