...
export async function GET(req: Request) {
  try {
    ...
    const rows = (data.list as unknown as Record<string, unknown>[]).map((r) => ({
      ...r,
      type: ASSET_TYPE_MAP[r.type as string] ?? r.type,
      status: ASSET_STATUS_MAP[r.status as string] ?? r.status
    }));
    const buf = await exportToXlsx(rows, ASSET_EXPORT_COLUMNS as unknown as Parameters<typeof exportToXlsx>[1], `assets-${new Date().toISOString().slice(0, 10)}.xlsx`);
