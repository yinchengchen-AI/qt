  ...
  try {
    ...
    const buf = await exportToXlsx(
      ...
      [
        ...
        { header: "发票类型", key: "invoiceType", width: 16, formatter: (v) => v ? (INVOICE_TYPE_MAP[v as string] ?? v as string) : "" },
        ...
        { header: "抬头类型", key: "titleType", width: 10, formatter: (v) => v ? (TITLE_TYPE_MAP[v as string] ?? v as string) : "" },
        ...
        { header: "状态", key: "status", width: 10, formatter: (v) => v ? ({DRAFT:"草稿",PENDING_FINANCE:"财务待审",ISSUED:"已开票",REJECTED:"已驳回",VOIDED:"已作废",RED_FLUSHED:"已红冲"}[v as string] ?? v as string) : "" }
      ],
      `发票列表_${ts}.xlsx`
    );
