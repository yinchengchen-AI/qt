...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const contract = await prisma.contract.findUnique({ where: { id: inv.contractId }, select: { contractNo: true } });
  const [applicant, finance, payments] = await Promise.all([
      prisma.user.findUnique({ where: { id: inv.applicantUserId }, select: { name: true, employeeNo: true } }),
      inv.financeUserId ? prisma.user.findUnique({ where: { id: inv.financeUserId }, select: { name: true, employeeNo: true } }) : null,
      prisma.payment.findMany({ where: { invoiceId: id, deletedAt: null }, select: { paymentNo: true, amount: true, receivedAt: true, status: true } })
    ]);
    ...
    const doc: PrintDoc = {
      ...
      mainRows: [
        ...
        { label: "发票类型", value: INVOICE_TYPE_MAP[inv.invoiceType] ?? inv.invoiceType },
        { label: "含税金额", value: Number(inv.amount).toFixed(2) },
        { label: "税率", value: (Number(inv.taxRate) * 100).toFixed(2) + "%" },
        ...
        { label: "抬头类型", value: TITLE_TYPE_MAP[inv.titleType] ?? inv.titleType },
