  ...
  try {
    ...
    if (parsed.type === "overview") {
      ...
      const buf = await exportToXlsx(rows, [
        ...
        { header: "金额", key: "value", width: 20, formatter: (v) => Number(v).toFixed(2) },
        { header: "数量", key: "count", width: 12 }
      ], `总览_${ts}.xlsx`);
      return new Response(new Uint8Array(buf), { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": `attachment; filename=overview_${ts}.xlsx` } });
    }
    if (parsed.type === "top-customers") {
      const data = await getTopCustomers(user, parsed.metric ?? "contract", 50);
      const buf = await exportToXlsx(data, [
        ...
        { header: "已回款额", key: "paymentTotal", width: 18, formatter: (v) => Number(v).toFixed(2) }
      ], `客户Top50_${ts}.xlsx`);
      return new Response(new Uint8Array(buf), { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": `attachment; filename=top-customers_${ts}.xlsx` } });
    }
    // sales-performance
    ...
    const buf = await exportToXlsx(data, [
      ...
      { header: "合同数", key: "contractCount", width: 10 },
      { header: "合同额", key: "contractAmount", width: 18, formatter: (v) => Number(v).toFixed(2) },
      { header: "已开票额", key: "invoiceAmount", width: 18, formatter: (v) => Number(v).toFixed(2) },
      { header: "已回款额", key: "paymentAmount", width: 18, formatter: (v) => Number(v).toFixed(2) }
    ], `业务员业绩_${ts}.xlsx`);
    return new Response(new Uint8Array(buf), { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": `attachment; filename=sales-performance_${ts}.xlsx` } });
  } catch (e) {
    return err(e);
  }
}
