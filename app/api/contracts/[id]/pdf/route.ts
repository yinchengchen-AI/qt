...
import { PAYMENT_METHOD_MAP, SERVICE_TYPE_MAP, CONTRACT_STATUS_MAP, PROJECT_STATUS_MAP, REVIEW_ACTION_MAP, INVOICE_STATUS_MAP, PAYMENT_STATUS_MAP } from "@/lib/enum-maps";
  ...
  try {
    ...
    const doc: PrintDoc = {
      ...
      mainRows: [
        ...
        { label: "不含税金额", value: fmtAmount(Number(c.amountExcludingTax)) },
        ...
        { label: "审批时间", value: c.reviewAt ? fmtDateTime(c.reviewAt) : "—" },
        { label: "审批意见", value: c.reviewComment ?? "—" }
      ],
      summary: [
        { label: "含税总额", value: fmtAmount(Number(c.totalAmount)), tone: "primary" },
        { label: "已开票", value: fmtAmount(t.invoicedAmount), tone: "warning" },
        { label: "未开票", value: fmtAmount(remainingInvoice), tone: "danger" },
        { label: "已回款", value: fmtAmount(t.paidAmount), tone: "success" },
        { label: "未回款", value: fmtAmount(remainingPayment), tone: "danger" }
      ],
      sections: [
        {
          title: "拆分项目",
          columns: ["项目编号", "项目名称", "起期", "止期", "工作流完成度", "状态"],
          rows: overview.projects.map((p) => {
            const completed = p.workflowCompleted;
            const total = p.workflowTaskCount;
            const pct = total > 0 ? ((completed / total) * 100).toFixed(1) + "%" : "—";
            return {
              项目编号: p.projectNo,
              项目名称: p.name,
