...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [recorder, reconciler] = await Promise.all([
      prisma.user.findUnique({ where: { id: p.recorderUserId }, select: { name: true, employeeNo: true } }),
      p.reconcileUserId ? prisma.user.findUnique({ where: { id: p.reconcileUserId }, select: { name: true, employeeNo: true } }) : null
    ]);
    ...
    const doc: PrintDoc = {
      ...
      mainRows: [
        ...
        { label: "到账日", value: new Date(p.receivedAt).toLocaleString("zh-CN") },
