...
import { getCustomer, listFollowUps, getCustomerOverview } from "@/server/services/customer";
...
import { CUSTOMER_STATUS_MAP, CONTRACT_STATUS_MAP, PROJECT_STATUS_MAP, INVOICE_STATUS_MAP, PAYMENT_STATUS_MAP } from "@/lib/enum-maps";
  ...
  try {
    ...
    const doc: PrintDoc = {
      ...
      meta: [
        ...
        { label: "客户负责人:", value: owner ? `${owner.name}(${owner.employeeNo})` : "—" }
      ],
      mainRows: [
      ...
      summary: [
        { label: "合同数", value: String(totals.contractCount), tone: "primary" },
        { label: "合同总额", value: fmtAmount(totals.contractTotal), tone: "primary" },
        { label: "已开票", value: fmtAmount(totals.invoicedTotal), tone: "warning" },
        { label: "已回款", value: fmtAmount(totals.paidTotal), tone: "success" }
      ],
      sections: [
        {
          title: "关联合同",
