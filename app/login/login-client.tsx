...
// 密码由 page.tsx (Server Component) 从 DEV_QUICK_FILL_PASSWORD 读入后通过 props 传入,
// 避免在 Client Component 中直接读取非 NEXT_PUBLIC_ 环境变量导致浏览器端拿不到。
...
function LoginForm({ quickFillPassword }: { quickFillPassword: string }) {
  ...
  function fillAccount(no: string) {
    ...
    form.setFieldsValue({ employeeNo: no, password: quickFillPassword });
    setError(null);
  }
...
export function LoginClient({ quickFillPassword }: { quickFillPassword: string }) {
  return (
          ...
          <div className={styles.formShell}>
            <section className={styles.formCard} aria-label="登录">
              <Suspense fallback={null}>
                <LoginForm quickFillPassword={quickFillPassword} />
              </Suspense>
            </section>
