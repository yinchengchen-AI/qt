...
export default function PerformancePage() {
  ...
  const contractChartData = rows.map(r => ({ name: r.name, value: r.contractAmount, type: "合同额" }));
  const invoiceChartData = rows.map(r => ({ name: r.name, value: r.invoiceAmount, type: "已开票" }));
  const paymentChartData = rows.map(r => ({ name: r.name, value: r.paymentAmount, type: "已回款" }));
