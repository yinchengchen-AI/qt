...
import { Button, Col, Row, Space, Tabs, Tag, Empty, theme, App as AntdApp } from "antd";
      ...
      <PageHeader
        ...
        subtitle={
          <Space>
            ...
            <span style={{ color: "var(--qt-text-faint)" }}>{data.code}</span>
            ...
            children: (
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} md={10}>
                  <ProCard title="基础信息">
                    <Row gutter={[8, 8]}>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>编号</Col>
                      <Col span={16}>{data.code}</Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>生效日期</Col>
                      <Col span={16}>{data.validFrom ? new Date(data.validFrom).toISOString().slice(0, 10) : "—"}</Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>到期日期</Col>
                      <Col span={16}>{data.validTo ? new Date(data.validTo).toISOString().slice(0, 10) : "—"}</Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>标签</Col>
                      <Col span={16}>
                        {(data.tags ?? []).length > 0
                          ? (data.tags ?? []).map((t) => <Tag key={t}>{t}</Tag>)
                          : "—"}
                      </Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>说明</Col>
                      <Col span={16}>{data.description || "—"}</Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>创建时间</Col>
                      <Col span={16}>{new Date(data.createdAt).toISOString().slice(0, 19).replace("T", " ")}</Col>
                      <Col span={8} style={{ color: token.colorTextTertiary }}>更新时间</Col>
                    ...
                    {data.attachments.map((a) => (
                      <Row key={a.id} align="middle" gutter={8}>
                        <Col flex="auto">
                          ...
                          <span style={{ color: token.colorTextTertiary, marginLeft: 8 }}>{(a.size / 1024).toFixed(1)} KB · {a.mimeType}</span>
                        </Col>
                        <Col>
                          <Button
                            size="small"
            ...
            children: (
              <ProCard>
                {logs?.list && logs.list.length > 0 ? (
                  <Space direction="vertical" style={{ width: "100%" }}>
                    {logs.list.map((l) => (
                      <div key={l.id} style={{ borderBottom: `1px dashed ${token.colorBorderSecondary}`, paddingBottom: 8 }}>
                        <Space>
                          <Tag color="blue">{l.action}</Tag>
                          <span style={{ color: token.colorTextTertiary }}>{new Date(l.at).toISOString().slice(0, 19).replace("T", " ")}</span>
                        </Space>
                        {l.diff ? (
                          <pre style={{ marginTop: 4, fontSize: 12, color: token.colorTextSecondary, background: token.colorBgLayout, padding: 8, borderRadius: 4, whiteSpace: "pre-wrap" }}>
