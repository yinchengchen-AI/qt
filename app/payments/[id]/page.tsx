...
export default function PaymentDetailPage() {
  ...
  return (
    <ProCard
      title={<span onClick={() => router.push("/payments")} style={{ cursor: "pointer" }}>← 回款 {data.paymentNo}</span>}
      extra={
        <Space>
    ...
    >
      ...
      {data.invoice && (
        <ProCard title="关联发票" style={{ marginTop: 16 }}>
      ...
      <ProCard title="分配明细" style={{ marginTop: 16 }}>
        <ProTable rowKey="id" search={false} options={false} pagination={false} dataSource={data.allocations ?? []} columns={[
          { title: "发票 ID", dataIndex: "invoiceId" }, { title: "项目 ID", dataIndex: "projectId" },
          { title: "金额", dataIndex: "amount", render: (v: any) => `¥${v}` },
          { title: "备注", dataIndex: "remark" }
        ]} />
      </ProCard>
