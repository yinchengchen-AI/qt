...
//   3. 签发 token, 返回 { ok: true } (不返回 token)
//   4. 同步写 OperationLog, action=PASSWORD_RESET_REQUESTED, diff 含 raw token +
//      完整 reset URL (供 admin 在 operation-logs 页面查看到后再人工送达)
//   5. 用户拿到 URL → 点击 → /api/auth/password-reset/confirm?token=xxx
//   6. confirm 接口校验 token + 写新密码 + 置 usedAt + audit
...
/**
 * 消费 token; 校验通过后必须置 usedAt (事务里完成)
 * 返回 ok 时 caller 应同时改密码 (建议同一事务)
 */
