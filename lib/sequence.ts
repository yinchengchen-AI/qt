...
// tx 传入时在调用方事务内执行（避免嵌套独立事务导致回滚跳号）；不传时保持原行为：自建事务。
export async function nextBusinessNo(type: SeqType, opts?: { yyyymm?: boolean }, tx?: Prisma.TransactionClient): Promise<string> {
  ...
  const run = async (t: Prisma.TransactionClient) => {
    ...
    const upserted = await t.sequence.upsert({
      where: { prefix_year: { prefix: key, year } },
      update: {},
      create: { prefix: key, year, lastValue: 0 }
    });
    // 2) 原子自增 + 返回新值（用 SQL RETURNING）
    const rows = await t.$queryRawUnsafe<Array<{ lastValue: number }>>(
